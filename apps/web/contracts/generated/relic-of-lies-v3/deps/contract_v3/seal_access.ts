...
/**
 * Seal Access Control for Love Letter Handles card ownership verification and
 * temporary access for Priest/Baron effects
 */

import { MoveStruct, normalizeMoveArguments, type RawTransactionArgument } from '../../../utils/index';
import { bcs } from '@mysten/sui/bcs';
import { type Transaction } from '@mysten/sui/transactions';
const $moduleName = 'contract_v3::seal_access';
...
export const CardOwnership = new MoveStruct({ name: `${$moduleName}::CardOwnership`, fields: {
        ...
        revealed_value: bcs.option(bcs.u8())
    } });
...
export function revealCard(options: RevealCardOptions) {
    ...
    const parameterNames = ["state", "cardIndex", "value"];
    return (tx: Transaction) => tx.moveCall({
        package: packageAddress,
        module: 'seal_access',
...
export interface GetRevealedValueArguments {
    state: RawTransactionArgument<string>;
    cardIndex: RawTransactionArgument<number | bigint>;
}
export interface GetRevealedValueOptions {
    package: string;
    arguments: GetRevealedValueArguments | [
        state: RawTransactionArgument<string>,
        cardIndex: RawTransactionArgument<number | bigint>
    ];
}
/** Get revealed value of card (if revealed) */
export function getRevealedValue(options: GetRevealedValueOptions) {
    const packageAddress = options.package;
    const argumentsTypes = [
        `${packageAddress}::seal_access::SealAccessState`,
        'u64'
    ...
    return (tx: Transaction) => tx.moveCall({
        package: packageAddress,
        module: 'seal_access',
        function: 'get_revealed_value',
        arguments: normalizeMoveArguments(options.arguments, argumentsTypes, parameterNames),
    });
}
export interface VerifyAccessArguments {
    state: RawTransactionArgument<string>;
...
export interface VerifyAccessOptions {
    package: string;
    arguments: VerifyAccessArguments | [
        state: RawTransactionArgument<string>,
        sealId: RawTransactionArgument<number[]>,
        caller: RawTransactionArgument<string>
    ];
...
/**
 * Helper function to verify access (called from sealed_game module) This is used
 * by the seal_approve_card entry function in sealed_game
 */
export function verifyAccess(options: VerifyAccessOptions) {
    const packageAddress = options.package;
    const argumentsTypes = [
        `${packageAddress}::seal_access::SealAccessState`,
        'vector<u8>',
    ...
    const parameterNames = ["state", "sealId", "caller"];
    return (tx: Transaction) => tx.moveCall({
        package: packageAddress,
        module: 'seal_access',
