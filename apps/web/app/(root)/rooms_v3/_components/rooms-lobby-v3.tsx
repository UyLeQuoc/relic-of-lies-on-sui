...
import { CreateRoomV3 } from "./create-room-v3";
import { toast } from "sonner";

type StatusFilter = "all" | "0" | "1" | "2" | "3" | "4";

export function RoomsLobbyV3() {
  ...
  const { joinRoom, isPending: isJoining } = useJoinRoomV3();
  ...
  const isPlayerInRoom = (room: SealedGameRoomType) => {
  ...
  const handleJoinRoom = async (roomId: string) => {
    ...
    try {
      ...
      router.push(`/game_v3?roomId=${roomId}`);
    } catch (err) {
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground flex items-center gap-2">
              ...
              Cards are encrypted with Seal - only you can see your hand
            </p>
          </div>
        </div>
        ...
        {!currentAccount && (
          <Card className="mb-6 border-violet-500/30">
            <CardHeader>
              ...
              <CardDescription>
                Please connect your wallet to view and join sealed game rooms
              </CardDescription>
            </CardHeader>
            <CardContent>
              <LoginComponent />
          ...
          <>
            ...
            {isLoadingRooms && rooms.length === 0 ? (
              <Card className="border-violet-500/30">
                <CardContent className="pt-6">
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">Loading sealed rooms...</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-violet-500/30">
                <CardContent className="pt-6">
                  <div className="text-center py-8">
                    <Lock className="w-12 h-12 mx-auto mb-4 text-violet-500/50" />
                    <p className="text-muted-foreground">
                      {rooms.length === 0
                        ? "No sealed rooms found. Create a room to get started!"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredRooms.map((room) => {
                  ...
                  const hasPendingAction = room.pending_action !== null;

                  return (
                    <Card key={room.id.id} className="flex flex-col border-violet-500/30 hover:border-violet-500/50 transition-colors">
                      <CardHeader>
                      ...
                      <CardContent className="flex-1 space-y-3">
                        ...
                        {inRoom && (
                          <Badge variant="secondary" className="w-fit bg-violet-500/20 text-violet-300">
                            You're in this room
                          </Badge>
                        )}
                      </CardContent>
