/// Sealed Game module for Love Letter with Seal Integration
/// Main game logic with encrypted cards and commitment verification
#[allow(lint(public_random), lint(self_transfer))]
module contract_v3::sealed_game;
...
use contract_v3::error;
use contract_v3::events;
use contract_v3::utils;
use contract_v3::seal_access::{Self, SealAccessState};
use contract_v3::leaderboard::{Self, Leaderboard};

// ============== Structs ==============

/// Player state in a game (card indices only, not values!)
...
public struct SealedGameRoom has key {
    id: UID,
...
public fun create_room(
    ...
    let room = SealedGameRoom {
        ...
        // Deck state (initialized at round start)
        deck_values: vector[],
        deck_secrets: vector[],
        commitments: vector[],
        deck_indices: vector[],
...
public fun start_round(
    ...
    room.deck_values = utils::create_deck();
    utils::shuffle_deck(&mut room.deck_values, random, ctx);
    
    // Generate secrets for each card
    let total_cards = room.deck_values.length();
    room.deck_secrets = utils::generate_secrets(random, total_cards, ctx);
    
    // Create commitments
    room.commitments = utils::create_deck_commitments(&room.deck_values, &room.deck_secrets);
    ...
    room.seal_access = seal_access::new(room.id.to_inner().to_bytes());
    ...
    if (num_players == 2) {
        (constants::two_player_public_cards() as u64).do!(|_| {
            ...
            if (card_idx_opt.is_some()) {
                ...
                room.public_cards.push_back(room.deck_values[card_idx]);
            };
        });
    };
    
    // Deal cards to players
    let mut dealt_indices: vector<vector<u64>> = vector[];
    
    num_players.do!(|i| {
        let mut player_cards: vector<u64> = vector[];
    ...
    if (first_card_opt.is_some()) {
        ...
        dealt_indices[0].push_back(card_idx);
...
public fun play_turn(
    ...
    // Verify commitment - THIS IS THE KEY SECURITY CHECK!
    assert!(error::is_valid_secret_length(&secret), error::invalid_secret());
    let commitment = &room.commitments[card_index];
    assert!(utils::verify_commitment(commitment, revealed_value, &secret), error::commitment_mismatch());
    
    events::emit_commitment_verified(room_id, card_index, revealed_value, true);
    ...
    if (has_countess && (has_king || has_prince)) {
        assert!(revealed_value == constants::card_countess(), error::must_discard_countess());
    };
    ...
    room.players[current_player_idx].discarded.push_back(revealed_value);
    
    // Remove from Seal access
    seal_access::remove_card(&mut room.seal_access, card_index);
    seal_access::reveal_card(&mut room.seal_access, card_index, revealed_value);
    
    // Track Spy play
    if (revealed_value == constants::card_spy()) {
    ...
    // Execute card effect
    let result = execute_card_effect(room, current_player_idx, revealed_value, target_idx, guess, random, ctx);
    
    events::emit_turn_played(room_id, sender, revealed_value, target_addr, guess, result);
...
public fun resolve_chancellor(
    ...
    events::emit_chancellor_resolved(room_id, sender, keep_card_index, return_indices.length());
...
fun execute_baron(
    ...
    // Player must reveal their remaining card for comparison
    // The attacker's remaining card value will need to be provided
    // For now, set pending - attacker's card value will be included in data
    // Note: In full implementation, attacker would reveal their card too
    ...
    let pending = PendingAction {
        ...
        responder: room.players[target].addr,
        responder_idx: target,
        data: vector[], // Will be filled when attacker reveals
        card_index: room.players[target].hand[0],
    };
    room.pending_action = std::option::some(pending);
    ...
    events::emit_pending_baron_response(
        room.id.to_inner(),
        room.players[player_idx].addr,
        0, // Placeholder - attacker needs to reveal
        room.players[target].addr,
    );
    
    b"Baron played - both players must reveal for comparison".to_string()
}
...
fun execute_prince(
    ...
    // Get target's card and force discard
    if (!room.players[target].hand.is_empty()) {
        let card_idx = room.players[target].hand.pop_back();
        let card_value = room.deck_values[card_idx];
        room.players[target].discarded.push_back(card_value);
        
        // Remove from Seal access
        seal_access::remove_card(&mut room.seal_access, card_idx);
        seal_access::reveal_card(&mut room.seal_access, card_idx, card_value);
        
        // Check if Princess was discarded
        if (card_value == constants::card_princess()) {
            eliminate_player(room, target, player_idx, constants::card_prince());
    ...
    if (new_card_opt.is_some()) {
        ...
        room.players[target].hand.push_back(new_card_idx);
        seal_access::register_card(&mut room.seal_access, new_card_idx, room.players[target].addr);
    } else if (room.burn_card_index.is_some()) {
        let burn_idx = *room.burn_card_index.borrow();
        room.players[target].hand.push_back(burn_idx);
        seal_access::register_card(&mut room.seal_access, burn_idx, room.players[target].addr);
...
fun execute_king(
    ...
    events::emit_hands_swapped(
        ...
        room.players[player_idx].addr,
        room.players[target].addr,
        target_card_idx, // Player now has target's card
        player_card_idx, // Target now has player's card
    );
...
fun is_player_in_room(room: &SealedGameRoom, addr: address): bool {
    let mut found = false;
...
fun all_others_immune(room: &SealedGameRoom, player_idx: u64): bool {
...
/// Check if player's hand contains a specific card value
fun hand_contains_value(room: &SealedGameRoom, player_idx: u64, value: u8): bool {
    let mut contains = false;
    room.players[player_idx].hand.do_ref!(|card_idx| {
        if (room.deck_values[*card_idx] == value) {
            contains = true;
        };
    });
    contains
...
fun eliminate_player(room: &mut SealedGameRoom, player_idx: u64, eliminator_idx: u64, card_used: u8) {
    room.players[player_idx].is_alive = false;
    
    // Reveal and discard all cards in hand
    let mut revealed_hand = vector[];
    while (!room.players[player_idx].hand.is_empty()) {
        ...
        revealed_hand.push_back(card_value);
...
fun advance_turn(room: &mut SealedGameRoom) {
    let num_players = room.players.length();
...
fun check_round_winner(room: &SealedGameRoom): std::option::Option<u64> {
...
fun check_spy_bonus(room: &SealedGameRoom): std::option::Option<u64> {
    ...
    num_players.do!(|i| {
        ...
        let holding_spy = room.players[i].is_alive && 
            ...
            room.deck_values[room.players[i].hand[0]] == constants::card_spy();
...
fun check_game_winner(room: &SealedGameRoom): std::option::Option<u64> {
    let num_players = room.players.length();
...
public fun room_id(room: &SealedGameRoom): ID {
    room.id.to_inner()
}

public fun room_name(room: &SealedGameRoom): std::string::String {
    room.name
}

public fun room_status(room: &SealedGameRoom): u8 {
    room.status
}

public fun room_current_turn(room: &SealedGameRoom): u64 {
    room.current_turn
}

public fun room_round_number(room: &SealedGameRoom): u8 {
    room.round_number
}

public fun current_player_address(room: &SealedGameRoom): address {
    let idx = room.current_turn % room.players.length();
    room.players[idx].addr
}

public fun player_hand_indices(room: &SealedGameRoom, player_addr: address): vector<u64> {
...
public fun player_discarded(room: &SealedGameRoom, player_addr: address): vector<u8> {
    let mut result = vector[];
    room.players.do_ref!(|p| {
        if (p.addr == player_addr) {
            result = p.discarded;
...
public fun player_tokens(room: &SealedGameRoom, player_addr: address): u8 {
    let mut result = 0u8;
    room.players.do_ref!(|p| {
        if (p.addr == player_addr) {
            result = p.tokens;
...
public fun player_is_alive(room: &SealedGameRoom, player_addr: address): bool {
...
public fun get_commitment(room: &SealedGameRoom, card_index: u64): vector<u8> {
    room.commitments[card_index]
}

public fun get_seal_namespace(room: &SealedGameRoom): vector<u8> {
    room.id.to_inner().to_bytes()
}

public fun deck_size(room: &SealedGameRoom): u64 {
    room.deck_indices.length()
}

public fun is_pending_action(room: &SealedGameRoom): bool {
    room.pending_action.is_some()
}

public fun is_chancellor_pending(room: &SealedGameRoom): bool {
    room.chancellor_pending
}

public fun all_players(room: &SealedGameRoom): vector<address> {
...
public fun cleanup_finished_room(registry: &mut RoomRegistry, room: &SealedGameRoom) {
...
#[test_only]
public fun get_card_value_for_testing(room: &SealedGameRoom, card_index: u64): u8 {
    room.deck_values[card_index]
}

#[test_only]
public fun get_card_secret_for_testing(room: &SealedGameRoom, card_index: u64): vector<u8> {
    room.deck_secrets[card_index]
}
